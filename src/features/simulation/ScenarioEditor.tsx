  ...
  return (
    <aside className="scenario-editor" aria-label="Service scenario settings">
      ...
      <div className="scenario-fields">
        ...
        <label>Service style<select aria-label="Service style" value={scenario.serviceStyle ?? 'À la carte'} onChange={(event) => onChange({ serviceStyle: event.target.value })}><option>À la carte</option><option>Buffet</option><option>Counter service</option><option>Room service</option></select></label>
        <label>Service variability<select aria-label="Service variability" value={scenario.variability ?? 'typical'} onChange={(event) => onChange({ variability: event.target.value as SimulationScenario['variability'] })}><option value="low">Low</option><option value="typical">Typical</option><option value="high">High</option></select></label>
      ...
      <details className="timing-settings"><summary>Advanced</summary><label>Deterministic seed<input aria-label="Deterministic seed" type="number" value={scenario.seed} onChange={(event) => onChange({ seed: Number(event.target.value) || 1 })} /></label></details>
    </aside>
  )
}
