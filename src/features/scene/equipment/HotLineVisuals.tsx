...
export function FlatTopFryerVisual({ widthM: w, depthM: d, heightM: h }: EquipmentVisualProps) {
  return <group>
    ...
    {[.26, .42].map((x, index) => <group key={index} position={[w * x, h * .83, 0]}>
      ...
      <mesh position={[0, .08, d * .14]} rotation={[Math.PI / 2, 0, 0]}><cylinderGeometry args={[.012, .012, d * .34, 8]} /><meshStandardMaterial color="#303936" metalness={.7} /></mesh>
    </group>)}
...
export function TandoorVisual({ widthM: w, depthM: d, heightM: h }: EquipmentVisualProps) {
  ...
  return <group>
    <mesh position={[0, h / 2, 0]} castShadow receiveShadow><cylinderGeometry args={[radius * .82, radius, h, 18]} /><meshStandardMaterial color="#aeb7b3" metalness={.55} roughness={.38} /></mesh>
...
export function CanopyHoodVisual({ widthM: w, depthM: d, heightM: h }: EquipmentVisualProps) {
  return <group>
    <BoxPart position={[0, h * .58, 0]} size={[w, h * .62, d]} color="#9ca8a4" metalness={.82} roughness={.22} />
