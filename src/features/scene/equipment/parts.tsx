...
export const STEEL = '#c7cfcc'
export const DARK_STEEL = '#5e6965'
export const BLACK = '#252b29'

export function BoxPart({ position, size, color = STEEL, metalness = .68, roughness = .3, children }: { position: [number, number, number]; size: [number, number, number]; color?: string; metalness?: number; roughness?: number; children?: ReactNode }) {
...
export function TubularLeg({ x, z, height, radius = .025 }: { x: number; z: number; height: number; radius?: number }) {
  return <mesh position={[x, height / 2, z]} castShadow><cylinderGeometry args={[radius, radius, height, 10]} /><meshStandardMaterial color={DARK_STEEL} metalness={.72} roughness={.3} /></mesh>
}

export function Handle({ x, y, z, vertical = true, length = .34 }: { x: number; y: number; z: number; vertical?: boolean; length?: number }) {
  return <group position={[x, y, z]}><mesh rotation={vertical ? [0, 0, 0] : [0, 0, Math.PI / 2]}><cylinderGeometry args={[.014, .014, length, 10]} /><meshStandardMaterial color="#303936" metalness={.7} roughness={.28} /></mesh></group>
}
