...
function cameraPreset(mode: CameraMode, architecture: Pick<Architecture, 'widthMm' | 'depthMm' | 'wallHeightMm'>, aspect: number, fovDegrees = 43, focusPoint?: PointMm) {
  ...
  const target: [number, number, number] = [focusPoint ? focusPoint.x / 1000 : width / 2, mode === 'top' ? 0 : Math.min(.72, wallHeight * .26), focusPoint ? focusPoint.y / 1000 : depth / 2]
  if (mode === 'top') {
    const halfFov = fovDegrees * Math.PI / 360
    const distance = focusPoint ? 3.4 : Math.max(depth / (2 * Math.tan(halfFov)), width / (2 * Math.max(.25, aspect) * Math.tan(halfFov))) * 1.18
    return { position: [target[0], Math.max(6, distance), target[2]] as [number, number, number], target, up: [0, 0, -1] as [number, number, number] }
  }
  const span = focusPoint ? 2.6 : Math.max(width, depth)
...
function CameraRig({ mode, fitSignal, architecture, focusPoint }: { mode: CameraMode; fitSignal: number; architecture: Architecture; focusPoint?: PointMm }) {
  const { camera, controls, size } = useThree()
  const architectureKey = cameraArchitectureKey(architecture)
  const { widthMm, depthMm, wallHeightMm } = architecture
  useEffect(() => {
    const preset = cameraPreset(mode, { widthMm, depthMm, wallHeightMm }, size.width / Math.max(1, size.height), 43, focusPoint)
    ...
    orbit?.update()
  }, [architectureKey, camera, controls, depthMm, fitSignal, focusPoint, mode, size.height, size.width, wallHeightMm, widthMm])
  return null
}
...
export function SceneCanvas({ architecture, cameraMode, fitSignal, focusPoint, walkMode = false, onContextLost, onContextRestored, children }: {
  ...
  return <Canvas shadows="basic" gl={{ alpha: false, antialias: true, depth: true, stencil: false, preserveDrawingBuffer: false, powerPreference: 'high-performance' }} camera={{ position: [6.2, 6.8, 8.2], fov: 43, near: .05, far: 80 }} dpr={[1, 1.5]}>
    ...
    <CameraRig mode={cameraMode} fitSignal={fitSignal} architecture={architecture} focusPoint={focusPoint} />
