...
const NumberField = ({ label, value, min = 0, onCommit }: { label: string; value: number; min?: number; onCommit(value: number): boolean | void }) => {
  const [draft, setDraft] = useState<string | null>(null)
  const savedValue = String(Math.round(value))
  const commit = (rawValue = draft ?? savedValue) => {
    const next = Number(rawValue)
    if (!rawValue.trim() || !Number.isFinite(next) || next < min) {
      setDraft(null)
      return
    }
    if (next === value) {
...
export function SpaceItemInspector({ store, selection, onClearSelection }: Props) {
  ...
  const apply = (next: Architecture) => store.getState().applySharedArchitecture(next).ok
  const remove = () => {
    if (!selection) return
    ...
    return (
      <aside className="inspector space-item-inspector" aria-label="Selected opening">
        ...
        <div className="field-pair">
          <NumberField label="Offset (mm)" value={opening.offsetMm} onCommit={(offsetMm) => patch({ offsetMm })} />
          <NumberField label="Width (mm)" value={opening.widthMm} min={1} onCommit={(widthMm) => patch({ widthMm })} />
          {opening.kind === 'door' && doorType === 'hinged' && <NumberField label="Swing depth (mm)" value={opening.swingDepthMm ?? opening.widthMm} min={1} onCommit={(swingDepthMm) => patch({ swingDepthMm })} />}
          {(opening.kind === 'window' || opening.kind === 'service-window') && <NumberField label="Sill height (mm)" value={opening.sillHeightMm ?? 900} onCommit={(sillHeightMm) => patch({ sillHeightMm })} />}
          {(opening.kind === 'window' || opening.kind === 'service-window') && <NumberField label="Opening height (mm)" value={opening.heightMm ?? 900} min={1} onCommit={(heightMm) => patch({ heightMm })} />}
        </div>
        {opening.kind === 'door' && <>
          <label>Door type
  ...
  if (selection.kind === 'pillar') {
    ...
    return (
      <aside className="inspector space-item-inspector" aria-label="Selected pillar">
        ...
        <div className="field-pair">
          <NumberField label="X (mm)" value={pillar.xMm} onCommit={(xMm) => patch({ xMm })} />
          <NumberField label="Y (mm)" value={pillar.yMm} onCommit={(yMm) => patch({ yMm })} />
          <NumberField label="Width (mm)" value={pillar.widthMm} min={1} onCommit={(widthMm) => patch({ widthMm })} />
          <NumberField label="Depth (mm)" value={pillar.depthMm} min={1} onCommit={(depthMm) => patch({ depthMm })} />
        </div>
        <div className="inspector-actions">
          <button type="button" className="danger-button" onClick={remove}>Remove</button>
        </div>
  ...
  return (
    <aside className="inspector space-item-inspector" aria-label="Selected zone">
      ...
      <div className="field-pair">
        <NumberField label="X (mm)" value={zone.xMm} onCommit={(xMm) => patch({ xMm })} />
        <NumberField label="Y (mm)" value={zone.yMm} onCommit={(yMm) => patch({ yMm })} />
        <NumberField label="Width (mm)" value={zone.widthMm} min={1} onCommit={(widthMm) => patch({ widthMm })} />
        <NumberField label="Depth (mm)" value={zone.depthMm} min={1} onCommit={(depthMm) => patch({ depthMm })} />
      </div>
