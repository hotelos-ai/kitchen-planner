...
const openingPatchSchema = z.object({
  ...
  doorType: z.union([z.enum(['hinged', 'double-hinged', 'sliding', 'double-sliding']), z.null()]).optional(),
...
const openingFields = (patch: boolean) => ({
  ...
  doorType: patch ? { type: ['string', 'null'], enum: ['hinged', 'double-hinged', 'sliding', 'double-sliding', null] } : enumSchema(['hinged', 'double-hinged', 'sliding', 'double-sliding']),
