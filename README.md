...
The supported user path is ChatGPT desktop's built-in browser when Site tools are available to the account: open the live URL, approve website access, and use the address-bar arrow to inspect available and recently used tools. For local Chrome development, enable `chrome://flags/#enable-webmcp-testing` and relaunch Chrome. Open **Use your AI agent**, confirm that it reports **Agent tools active** and 26 registered tools, then copy the starter prompt into your agent conversation. The deployment deliberately does not ship an Origin-Trial token; the Chrome flag is a developer test path, while ChatGPT desktop provides the supported user path. Its `Permissions-Policy: tools=(self)` header keeps WebMCP permission on the top-level page and same-origin descendants without delegating it cross-origin.
...
| `share_results` | Produces selected `plan`, `metrics`, `findings`, and `assumptions` report sections and/or a portable workspace link for one or more layouts. The legacy `report` value still selects every section. Links contain a compressed, validated project snapshot and restore in a fresh browser without prior local storage. |
