...
test('keeps every 3D control usable and recovers a lost WebGL context', async ({ page }) => {
  ...
  const planSurface = page.locator('[data-workspace-surface="plan"]')
  const planBeforeResize = await planSurface.boundingBox()
  const divider = page.getByRole('separator', { name: 'Resize split view' })
  const dividerBounds = await divider.boundingBox()
  expect(planBeforeResize).not.toBeNull()
  ...
  await page.mouse.move(dividerBounds!.x + 140, dividerBounds!.y + dividerBounds!.height / 2)
  await page.mouse.up()
  await expect.poll(async () => (await planSurface.boundingBox())?.width ?? 0).toBeGreaterThan(planBeforeResize!.width + 100)
  await page.getByRole('button', { name: 'Top', exact: true }).click()
  await page.getByRole('button', { name: 'Fit room', exact: true }).click()
  expect(pageErrors).toEqual([])
})
